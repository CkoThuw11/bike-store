...
def get_brand_service(
    repo: IBrandRepository = Depends(get_brand_repository)
) -> BrandService:
...
def get_store_service(
    repo: IStoreRepository = Depends(get_store_repository)
) -> StoreService:
...
def get_product_service(
    product_repo: IProductRepository = Depends(
        get_product_repository
    ),
    stock_repo: IStockRepository = Depends(
        get_stock_repository
    ),
    store_repo: IStoreRepository = Depends(
        get_store_repository
    ),
...
def get_order_service(
    ...
    return OrderService(repo, order_item_repo, customer_service, store_service, staff_service, stock_service)
